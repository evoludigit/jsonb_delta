...
cmd_profile() {
    ...
    ip="$(server_ip)"
    {
        ...
        [[ $DRY_RUN -eq 0 ]] && {
            echo "cpu:           $(ssh "root@$ip" "lscpu | sed -n 's/^Model name: *//p'" 2>/dev/null || echo unknown)"
            echo "vcpu/ram:      $(ssh "root@$ip" "nproc" 2>/dev/null || echo '?') vCPU / $(ssh "root@$ip" "free -g | awk '/^Mem:/{print \$2}'" 2>/dev/null || echo '?') GB"
            echo "postgresql:    $(ssh "root@$ip" "psql --version" 2>/dev/null || echo unknown)"
        }
...
"no server named '$BENCH_NAME' — nothing to measure.
  Provision one first (this bills):
    ...
    rsync -az --exclude target --exclude .git ./ "root@$ip:/opt/jsonb_delta/"
...
sudo -u postgres psql -d bench -v ON_ERROR_STOP=1 -f test/bench/scenarios_changeset.sql
sudo -u postgres psql -d bench -c 'SELECT count(*) FROM bench.run_all();'
EOF

    local stamp
    stamp="$(date -u +%Y-%m-%d)"
    ssh "root@$ip" "sudo -u postgres psql -d bench -c 'SELECT * FROM bench.report;'" \
        > "$RESULTS_DIR/results-$stamp-$BENCH_TYPE.txt"
    log "results written to $RESULTS_DIR/results-$stamp-$BENCH_TYPE.txt"
